...
model_mapping <- sort(list.files("input/raw/", pattern = "model_mapping.*dat"), decreasing = T)
file.copy(from = paste0("input/raw/",model_mapping[1]), 
          to = "input/model_mapping.dat", overwrite = T)

# Control -----------------------------------------------------------------

latest_control <- sort(list.files("input/raw/", pattern = "INPUT_LUNCH_DATE.*dat"), decreasing = T)
file.copy(from = paste0("input/raw/",latest_control[1]), 
          to = "input/latest_control.dat", overwrite = T)

# Latest Sales ------------------------------------------------------------

latest_sales <- sort(list.files("input/raw/", pattern = "INPUT_SALE_DATA.*dat"), decreasing = T)
